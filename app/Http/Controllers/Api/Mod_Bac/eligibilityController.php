...
class eligibilityController extends Controller
{
    ...
    {
        $main['main'] = db::table($this->Bac . '.eligibility_main')->where('id', $id)->get();
        $main['remarks'] = db::table($this->Bac . '.eligibility_remarks')->where('main_id', $id)->get();
        return response()->json(new JsonResponse($main));
    }
    public function cancel($id)
    {
        db::table($this->Bac . '.eligibility_main')->where('id', $id)->update(['stat' => 1]);
    ...
    public function store(Request $request)
    {
        ...
        //     $id = $form['id'];
        //     if ($form['id'] > 0) {
        //         db::table($this->Bac . '.eligibility_main')->where('id', $form['id'])->update($form);
        //     } else {
        //         db::table($this->Bac . '.eligibility_main')->insert($form);
        //         $id = $this->G->pk();
        //     }
        //     db::table($this->Bac . '.eligibility_remarks')->where('main_id', $form['id'])->delete();
        ...
        try {
            ...
            $data = array(
                ...
                'eligibility_description' => $request->eligibility_description,
                'mop' => $request->mop,
                'abc_a' => $request->abc_a,
                'abc_b' => $request->abc_b,
                'abc_c' => $request->abc_c,
                'remarks_others' => $request->remarks_others,
            ...
            DB::beginTransaction();
            if ($idx == 0) {
                db::table($this->Bac . '.eligibility_main')->insert($data);
                $idx = $this->G->pk();
            } else {
                $data['upid'] = Auth::user()->id;
                db::table($this->Bac . '.eligibility_main')->where('id', $idx)->update($data);
            }
