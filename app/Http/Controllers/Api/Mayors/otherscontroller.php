...
class otherscontroller extends Controller
{
    ...
    {
        $query = DB::select("SELECT CONCAT(LPAD(COUNT(*)+1,4,0),'-',DATE_FORMAT(NOW(),'%m%d%Y'))as 'NOS' FROM " . $this->mayors_db . ".other_transaction");
        return response()->json(new JsonResponse(['data' => $query]));
    }
    public function printform($id) {
        $datax = db::select('call '.$this->mayors_db.'.get_other_transaction(?)',[$id]);
        ...
        $template_file_name = public_path().'\MAYORS\authoritytotravel.docx';
        ...
        if($zip_val->open($full_path) == true)
        {
            ...
            $message = $zip_val->getFromName($key_file_name); 
              ...
              $message = str_replace("@destination",$row->destination,$message);
              $message = str_replace("@purpose",$row->purpose,$message);
              $message = str_replace("@dateofvalidity",$row->dateto,$message);
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($id == 0) {
                ...
                db::table($this->mayors_db .'.other_transaction')->insert($form);
            } else {
                Log::debug('update');
                $form['upidint'] = Auth::user()->id;
                db::table($this->mayors_db .'.other_transaction')
                    ->where('id', $id)
                    ->update($form);
            }
    ...
    public function show(Request $request)
    {
        try {
            $from = $request->from;
            $to = $request->to;
            $data =  db::table($this->mayors_db .'.other_transaction')
