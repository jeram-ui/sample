...
use ZipArchive;
class returningresidence extends Controller
{
    private $lgu_db;
    private $hr_db;
    private $trk_db;
    ...
    {
        $query = DB::select("SELECT CONCAT(LPAD(COUNT(*)+1,4,0),'-',DATE_FORMAT(NOW(),'%m%d%Y'))as 'NOS' FROM " . $this->mayors_db . ".returning_residence");
        return response()->json(new JsonResponse(['data' => $query]));
    }
    public function printform($id) {
        $datax = db::select('call '.$this->mayors_db.'.returning_residence(?)',[$id]);
        ...
        $template_file_name = public_path().'\MAYORS\returningresidence.docx';
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($id == 0) {
                ...
                db::table($this->mayors_db .'.returning_residence')->insert($form);
            } else {
                Log::debug('update');
                $form['upidint'] = Auth::user()->id;
                db::table($this->mayors_db .'.returning_residence')
                    ->where('id', $id)
                    ->update($form);
            }
    ...
    public function show(Request $request)
    {
        try {
            $from = $request->from;
            $to = $request->to;
            $data =  db::table($this->mayors_db .'.returning_residence')
