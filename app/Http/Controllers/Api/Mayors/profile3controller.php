...
class profile3controller extends Controller
{
        ...
        try { 
          ...
          $idx=$mainData['pkid'];
            if ($idx > 0) {
                ...
                $this->save($mainData,$cedulaData,$requirements,$unit,$route,$reference,$cc,$formid);
            }; 
              DB::commit();
    ...
    public function save($mainData,$cedulaData,$requirements,$unit,$route,$reference,$cc,$frmid)
    {  
      db::table($this->lgu_db.'.ebplo_tbl_profile')->insert($mainData);
      $id = DB::getPDo()->lastInsertId(); 
      $this->save_details($id,$mainData,$cedulaData,$requirements,$unit,$route,$reference,$cc,$frmid);
    } 
    public function save_details($id,$mainData,$cedulaData,$requirements,$unit,$route,$reference,$cc,$frmid)
    {
        //Billing
        $fees= DB::select('Call '.$this->lgu_db.'.ebplo_display_accounts_jho(?)', array($frmid));
        ...
        foreach($fees as $row){  
          if($row->Type_ === 'FIXED'){
            $billing = array(
              ...
              'transaction_type'=>"Trisikad Permit",
              'bill_number'=>$mainData['appno'],
              'payer_id'=> ($mainData['appbusinessno'] === null) ? 0 : $mainData['appbusinessno'],
              'business_application_id' ($mainData['appid'] === null) ? 0 : $mainData['appid'],
    ...
    { 
        ...
        DB::delete('DELETE FROM '.$this->lgu_db.'.cto_general_billing WHERE transaction_type = ? and ref_id = ? and bill_id = ?',['Trisikad Permit',$idx,$idx]);
 ...
 public function printmasterlist(Request $request)
 {
   ...
   try {
    ...
    if ($frm === 'Trisikad Permit') {
     ...
     <td style="border:0.5px solid black;text-align:left;width:10%;">' . $row['appliedname'] . '</td>
     ...
     ';                       
     PDF::SetTitle('Trisikad Permit Master List');
 ...
 {
  $summary = DB::select('Call '.$this->lgu_db.'.trisikad_summarycount_zoe()');
  ...
  $logo = config('variable.logo');
   try {
    ...
    ';   
     PDF::SetTitle('Trisikad Summary Count');
     PDF::AddPage();
     PDF::writeHTML($html_content, true, true, true, true, '');
     PDF::Output(public_path() . '/prints.pdf', 'F');
     return response()->json(new JsonResponse(['status' => 'success']));
   ...
   $dtls = DB::select('Select * from '.$this->lgu_db.'.ebplo_tbl_profile_unitdetails where mainid = '.$request->pkid);  
    ...
    $logo = config('variable.logo');
     try {
       ...
       <h3 align="center">MAYOR`S TriSikad PERMIT(MTSP)</h3>
       <br><br><br>
       <table>
       <tr>
       ...
       <tr>
       <th  style="width:2%;"></th>
       <th style="width:98%;">By virtue of Ordinance No. 2009-2006 series of 2009 of the '.$name.', Applicant/Operator</th>
       </tr>
       <tr>
       <th  style="width:2%;"></th>
       <th style="width:98%;">is hereby authorized to operate the following Trisikad units:</th>
       </tr>
       </table>
       <br></br>
       <br></br>
       ...
       <tr>
       <th style="width:5%;"></th>
       <th style="width:90%;border:0.5px solid black;text-align:center;background-color:#dedcdc;line-height:15px;"><b>BODY NO.</b></th>
       <td style="width:5%;"></td>
       
       </tr>';
       $bc = '';
       foreach($dtls as $row){  
        if($bc == ''){ $bc = $row->body_case; }else{ $bc .= ', '.$row->body_case; } 
      }
       $html_content .= '
       <tr>
       <td style="width:5%;"></td>
       <td style="width:90%;border:0.5px solid black;text-align:center;">'.$bc.'</td>
       <td style="width:5%;"></td>
       </tr>';
       $html_content .= '
       </table>
       ...
       <tr>
       <th  style="width:5%;"></th>
       <th style="width:95%;">2. Applicant/Operator shall employ a driver who is of legal age, a resident and registered voter of the</th>
       </tr>
       <tr>
       <th  style="width:5%;"></th>
       ...
       <tr>
       <th  style="width:5%;"></th>
       <th style="width:95%;">3. Applicant/Operator shall only ply on the route assigned in this Permit;</th>
       </tr>      
       <tr>
       <th  style="width:5%;"></th>
       <th style="width:95%;">4. Applicant/Operator shall pay in full the required Permit fee;</th>
       </tr>
       <tr>
       <th  style="width:5%;"></th>
       <th style="width:95%;">5. Applicant/Operator/Driver shall strictly observe and comply with the policies, ordinance, rules and</th>
       </tr>
       <tr>
       <th  style="width:5%;"></th>
       <th style="width:95%;">regulations of the '.$type.' Government of '.$name.', and all other applicable law of the</th>
       </tr>
       <tr>
       <th  style="width:5%;"></th>
       ...
       <tr>
       <th  style="width:5%;"></th>
       <th style="width:95%;">6. Any violation of the conditions set forth herein shall be sufficient cause for the automatic revocation </th>
       </tr>
       <tr>
       <th  style="width:5%;"></th>
       <th style="width:95%;"><span>of this Permit.</span></th>
       </tr>
       <br><br>
       <tr>
       <th style="width:5%;"></th>
