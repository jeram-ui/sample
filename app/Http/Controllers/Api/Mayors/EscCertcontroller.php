...
use ZipArchive;
class EscCertcontroller extends Controller
{
    private $lgu_db;
    private $hr_db;
    private $trk_db;
    ...
    public function ref(Request $request)
    {
        ...
        $query = DB::select("SELECT CONCAT(LPAD(COUNT(*)+1,4,0),'-',DATE_FORMAT(NOW(),'%m%d%Y'))as 'NOS' FROM " . $this->mayors_db . ".esc_cert");
        return response()->json(new JsonResponse(['data' => $query]));
    }
    public function printform($id) {
       
        $datax = db::table($this->mayors_db.'.esc_cert')
        ...
        db::raw("(CASE WHEN `gender` = 'Male' THEN CONCAT('Mr. ',`qpsii_lgusystem`.`fn_upper_case`(`lname`))
ELSE  CONCAT('Mrs. ',`qpsii_lgusystem`.`fn_upper_case`(`lname`))
 END)AS'genderx'")
,db::raw("DATE_FORMAT(`trans_date`,'%M %Y') AS niceDate ")
,db::raw("DATE_FORMAT(`trans_date`,'%D') AS day") 
        )

        ->where('id',$id)->first();

        Log::debug($datax->trans_date);
        $template_file_name = public_path().'\MAYORS\esc certificate.docx';
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($id == 0) {
                ...
                db::table($this->mayors_db . '.esc_cert')->insert($form);
            } else {
                Log::debug('update');
                $form['upidint'] = Auth::user()->id;
                db::table($this->mayors_db . '.esc_cert')
                    ->where('id', $id)
                    ->update($form);
            }
    ...
    public function show(Request $request)
    {
        try {
            $from = $request->from;
            $to = $request->to;
            $data =  db::table($this->mayors_db . '.esc_cert')
