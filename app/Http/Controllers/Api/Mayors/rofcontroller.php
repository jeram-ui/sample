...
class rofcontroller extends Controller
{
    ...
    {
        $query = DB::select("SELECT CONCAT(LPAD(COUNT(*)+1,4,0),'-',DATE_FORMAT(NOW(),'%m%d%Y'))as 'NOS' FROM " . $this->mayors_db . ".returning_overseas_filipinos");
        return response()->json(new JsonResponse(['data' => $query]));
    }
    public function printform($id) {
        $datax = db::select('call '.$this->mayors_db.'.returning_overseas_filipinos(?)',[$id]);
        ...
        $template_file_name = public_path().'\MAYORS\returningoverseasfilipinos.docx';
        ...
        if($zip_val->open($full_path) == true)
        {
            ...
            $message = $zip_val->getFromName($key_file_name); 
               ...
               $message = str_replace("@ref",$row->ref_no,$message);
               $message = str_replace("@purok",$row->barangay_name,$message); 
               $message = str_replace("@name",$row->name,$message);
               $message = str_replace("@address",$row->address,$message);
               //$message = str_replace("@strandedat",$row->strandedat,$message);
               $message = str_replace("@day",$row->DAY,$message);
               $message = str_replace("@month",$row->datefrom,$message);
               $message = str_replace("@brgycap",$row->capt,$message);
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($id == 0) {
                ...
                db::table($this->mayors_db .'.returning_overseas_filipinos')->insert($form);
            } else {
                Log::debug('update');
                $form['upidint'] = Auth::user()->id;
                db::table($this->mayors_db .'.returning_overseas_filipinos')
                    ->where('id', $id)
                    ->update($form);
            }
    ...
    public function show(Request $request)
    {
        try {
            $from = $request->from;
            $to = $request->to;
            $data =  db::table($this->mayors_db .'.returning_overseas_filipinos')
                ->select('*', db::raw("TRIM(CONCAT(`firstname`,' ',ifnull(`middlename`,''),' ',`lastname`,' ',ifnull(`suffix`,'')))AS 'name'"))
                ->where('stat', 0)
