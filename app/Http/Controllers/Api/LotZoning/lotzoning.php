...
class lotzoning extends Controller
{
    private $lgu_db;
    private $hr_db;
    private $trk_db;
    ...
    public function displaylist(Request $request)
    {
        $from = $request->from;
        $to =  $request->to;
        $list = DB::select('call ' . $this->lgu_db . '.jay_ebplo_display_ecpdc_application_form_cert_gigil(?,?,?)', array($from,$to,'Lot Zoning'));
    ...
    public function store(Request $request)
    {
        ...
        unset($main['lotid']);
        unset($main['tdid']);
        if ($main['frm_name']=='Lot Zoning With Variance') {
            unset($main['existing']);
        }else{
    ...
    {
        $data = DB::select('call '.$this->lgu_db.'.jay_ebplo_print_ecpdc_application_form_cert_gigil(?,?)',array($id,'Lot Zoning'));
        ...
        <br>
        <h2 align="center" style="line-height: 5px"> LOT ZONING CERTIFICATION </h2>
        <style>
        table{
            width:100%;
            padding:3px;
    ...
    public function printLotzoninglist(Request $request)
    {
        ...
        <tr>
            ...
            foreach ($data as $row) {
                ...
                <td width="10%" align="left"> ' . $row['bns_tdno'] . '</td>
                <td width="13%">' . $row['Certification Purpose'] . '</td>
                <td width="10%">' . $row['Zoning Fee'] . '</td>
            </tr>';
            ...
            <table width="100%">
            <tr>
                <th style="width:88%;text-align:left;border: 1px solid black;"> TOTAL >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>></th>  
                <th style="width:10%;text-align:center;height:15px;border: 1px solid black;">'.number_format($TotalAmnt,2).'</th>  
            </tr> 
            <tr>
