...
namespace App\Http\Controllers\Api\TricyclePermit;
...
class tricycleController extends Controller
{
    ...
    {
      $list = DB::select('Call '.$this->lgu_db.'.profile_requirements_zoe()');
      return response()->json(new JsonResponse($list));
    }
    public function tricyclelistdisplay( Request $request)
    {
  ...
  {
      //Billing
      $fees= DB::select('Call '.$this->lgu_db.'.ebplo_display_accounts_jho(16)');
      ...
      foreach($fees as $row){  
        if($row->Type_ === 'FIXED'){
          $billing = array(
            ...
            'transaction_type'=>"Tricycle Permit",
  ...
  { 
      ...
      DB::delete('DELETE FROM '.$this->lgu_db.'.cto_general_billing WHERE transaction_type = ? and ref_id = ? and bill_id = ?',['Tricycle Permit',$idx,$idx]);
 ...
 public function printtricyclemasterlist(Request $request)
 {
   ...
   try {
     $main=$request->main;
       ...
       $html_content = '
             ' . $logo . ' 
       <h3 align="center">TRICYCLE PERMIT MASTER LIST</h3>
       <table>
       <tr>
     ...
     ';                       
     PDF::SetTitle('Tricyle Permit Master List');
 ...
 {
  $summary = DB::select('Call '.$this->lgu_db.'.tricycle_summarycount_zoe()');
  ...
  $logo = config('variable.logo');
   try {
    ...
    ';   
     PDF::SetTitle('Tricyle Summary Count');
     PDF::AddPage();
     PDF::writeHTML($html_content, true, true, true, true, '');
     PDF::Output(public_path() . '/prints.pdf', 'F');
     return response()->json(new JsonResponse(['status' => 'success']));
 ...
 public function printTricyclePermit(Request $request)
 {
   $dtls = DB::select('Select * from '.$this->lgu_db.'.ebplo_tbl_profile_unitdetails where mainid = '.$request->pkid);  
    ...
    $logo = config('variable.logo');
     try {
       ...
       <h3 align="center">MAYOR`S TRICYCLE PERMIT(MTCP)</h3>
       <br><br><br>
       <table>
       <tr>
       ...
       <tr>
       <th  style="width:5%;"></th>
       <th style="width:95%;">Applicant/Operator is hereby authorized to operate the following Tricycle units:</th>
       </tr>
       </table>
       <br></br>
       <br></br>
