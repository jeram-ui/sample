...
use Storage;
use File;
class bploControler extends Controller
{
    private $lgu_db;
    private $hr_db;
    private $trk_db;
    ...
    public function show(Request $request){
        $list = db::table($this->ins_db.'.bplo_inspectorate_main')
        ->where('stat',0)
    ...
    public function cancel($id){
        db::table($this->ins_db.'.bplo_inspectorate_main')->where('id',$id)->update(['stat'=>1]);
    ...
    {
        $pre = 'BIR';
        $table = $this->ins_db . ".bplo_inspectorate_main";
        $date = $request->date;
        $refDate = 'date_time';
    ...
    public function update($id){
        ...
        $main['form']=   db::table($this->ins_db.'.bplo_inspectorate_main')->where('id',$id)->get();
    ...
    public function store(Request $request){
     try {
        ...
        $id = $form['id'];
       if ($id == 0) {
          db::table($this->ins_db.'.bplo_inspectorate_main')->insert($form);
          $id = $this->G->pk();
       }else{
          db::table($this->ins_db.'.bplo_inspectorate_main')->where('id',$id)->update($form);
       }

       db::table($this->ins_db.'.bplo_inspectorate_details')->where('main_id', $id)->delete();
       foreach ($dtls as $key => $value) {
         $data = array(
              'main_id'=> $id,
              'req_id'=>$value['id'],
              'for_compliance'=>$value['for_compliance'],
              'compliance_day'=>$value['compliance_day'],
              'Complying'=>$value['Complying'],
              'Non_Complying'=>$value['Non_Complying'],
              'remarks'=>$value['remarks'],
         );
         db::table($this->ins_db.'.bplo_inspectorate_details')->insert($data);
       }
    ...
    public function getReq(Request $request ){
        $list = db::table($this->ins_db.'.bplo_inspection_req')
        ->get();
        return response()->json(new JsonResponse($list));
    }
