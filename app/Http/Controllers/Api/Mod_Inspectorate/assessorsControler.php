...
use Storage;
use File;
class assessorsControler extends Controller
{
    private $lgu_db;
    private $hr_db;
    private $trk_db;
    ...
    public function show(Request $request){
        $list = db::table($this->ins_db.'.assessors_inspection')
        ->where('stat',0)
    ...
    public function cancel($id){
        db::table($this->ins_db.'.assessors_inspection')->where('id',$id)->update(['stat'=>1]);
    ...
    {
        $pre = 'AIR';
        $table = $this->ins_db . ".assessors_inspection";
        $date = $request->date;
        $refDate = 'date_time';
    ...
    public function update($id){
        ...
        $main['form']=   db::table($this->ins_db.'.assessors_inspection')->where('id',$id)->get();
    ...
    public function store(Request $request){
     try {
        ...
        $id = $form['id'];
       if ($id == 0) {
          db::table($this->ins_db.'.assessors_inspection')->insert($form);
          $id = $this->G->pk();
       }else{
          db::table($this->ins_db.'.assessors_inspection')->where('id',$id)->update($form);
       }
