...
class legalOpinionController extends Controller
{
    ...
    public function getRef(Request $request)
    {
        // dd($request);
        $pre = 'LON';
        $table = $this->lgu_db . ".law_legal_opinion";
        $date = $request->date;
        $refDate = 'trans_date';
    ...
    {
        $list = db::table($this->lgu_db . '.law_legal_opinion')
            ->select('*', db::raw("(CASE WHEN entry_type = 'Barangay' THEN barangay_name
            WHEN entry_type = 'Department' THEN `req_dept`
            WHEN entry_type = 'Person' THEN CONCAT(lname,', ',fname,' ',mname)
             END) AS names"))
            ->where('status', 0)
    ...
    {
        db::table($this->lgu_db . '.law_legal_opinion')->where('id', $id)->update(['with_case' => '1']);
    ...
    public function store(Request $request)
    {
        try {
            ...
            if ($idx == 0) {
                db::table($this->lgu_db . '.law_legal_opinion')->insert($main);
            } else {
                db::table($this->lgu_db . '.law_legal_opinion')->where('id', $idx)->update($main);
            }
            return response()->json(new JsonResponse(['Message' => 'Transaction Successfully Completed.', 'status' => 'success']));
        } catch (\Exception $err) {
    ...
    {
        $data['main'] = DB::table($this->lgu_db . '.law_legal_opinion')->where('id', $id)->get();
        return response()->json(new JsonResponse($data));
    }
    public function cancel($id)
    {
        DB::table($this->lgu_db . '.law_legal_opinion')->where('id', $id)->update(['status' => '1']);
        return response()->json(new JsonResponse(['Message' => 'Deleted Successfully.', 'status' => 'success']));
    }
    public function printform($id)
    {
        $data = db::table($this->lgu_db . '.law_legal_opinion')->where('id', $id)
