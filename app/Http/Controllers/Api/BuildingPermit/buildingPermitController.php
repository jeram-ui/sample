...
namespace App\Http\Controllers\Api\BuildingPermit;
...
class buildingPermitController extends Controller
{
    ...
    public function buildingpermitNoNew (Request $request)
    {     
      $list = DB::select('call ' . $this->lgu_db . '.balodoy_get_buildingpermitno_new()');   
      return response()->json(new JsonResponse($list));
    } 
    ...
    public function getbuildingPermitList(Request $request)
    {           
      $tmp = json_decode($request->dates);
      $dateFrom = $tmp->from ;
      $dteTo = $tmp->to ;
      $issueType = $request->issuanceType;        
      $list = DB::select('call ' . $this->lgu_db . '.balodoy_display_buildingpermit_all(?,?,?)', array($dateFrom,$dteTo,$issueType));
      return response()->json(new JsonResponse($list));
    }
    ...
    public function store(Request $request)
    {
      try {        
        DB::beginTransaction();                         
        $buildingMain = $request->buildingMainData;          
        $box1 = $request->box1Data;                                  
        $materialsDetails = $request->materialsInfoDetails;          
        ...
        $idx=$buildingMain['application_id'];                               
        if ($idx > 0) {                       
            $this->update($idx,$buildingMain,$box1,$materialsDetails,$box2, $box3, $box4, $box5, $box6, $box7);
        }else {
            $this->save($buildingMain,$box1,$materialsDetails,$box2, $box3, $box4, $box5, $box6, $box7);
        };
    ...
    {         
        $data['buildingMain'] = DB::select('call ' . $this->lgu_db . '.balodoy_get_buildingpermit_main(?)', array($id));      
        $data['buildingbox1'] = DB::select('call ' . $this->lgu_db . '.balodoy_get_buildingpermit_box1(?)', array($id));       
        $data['buildingMaterials'] = DB::select('call ' . $this->lgu_db . '.balodoy_get_buildingpermit_materialsdetails(?)', array($id));
        $data['buildingbox2'] = DB::select('call ' . $this->lgu_db . '.balodoy_get_buildingpermit_box2(?)', array($id));
        $data['buildingbox3'] = DB::select('call ' . $this->lgu_db . '.balodoy_get_buildingpermit_box3(?)', array($id));
        $data['buildingbox4'] = DB::select('call ' . $this->lgu_db . '.balodoy_get_buildingpermit_box4(?)', array($id));   
        $data['buildingbox5'] = DB::select('call ' . $this->lgu_db . '.balodoy_get_buildingpermit_box5(?)', array($id));  
        // $data['buildingbox6'] = DB::select('call ' . $this->lgu_db . '.balodoy_get_business_new_bspinfodetails(?)', array($id));
        $data['buildingbox7'] = DB::select('call ' . $this->lgu_db . '.balodoy_get_buildingpermit_box7(?)', array($id));    
        return response()->json(new JsonResponse($data));        
    }
    public function save($buildingMain,$box1,$materialsDetails,$box2, $box3, $box4, $box5, $box6, $box7)
    {        
        if ($box1['bp_ownedConstructionID'] > 0) {
            $appType = 'BUSINESS';
            $appID = $box1['bp_ownedConstructionID'];
            $appName = $box1['bp_ownedConstruction'];
            } else {
            $appType = 'PERSON';           
            $appID = $box1['bp_applicantID'];
            $appName = $box1['bp_applicantName'];
            };        
        $newBuilding = array(        
            'permit_type' => 1,
            ...
            'applicant_id' =>  $appID,
            'application_code' => $buildingMain['bp_applicationNo'],
            'area_code' => $buildingMain['bp_areaNo'],
            'applicant_address' => $box1['bp_addBarangay'], 
            'lot_no' => $box1['bp_locLotNo'],
            'blk_no' => $box1['bp_locBlockNo'], 
            'tct_no' => $box1['bp_TCTNo'],
            'property_st' => $box1['bp_locStreet'],
            'property_brgy' => $box1['bp_locBarangay'],
            'property_city' => $box1['bp_locCityMun'],
            'with_td' => 'NO',
            'indigens_status' => 'NO',
            'with_bp' => 'NO',
            'td_no' => $box1['bp_TDNo'],
        ...
        DB::table($this->lgu_db.'.eceo_application')->insert($newBuilding);
        $buildingID = DB::getPDo()->lastInsertId();
        if ($box1['bp_scopeofWork'] == "New Construction") {
            $scopeID = 1;          
        } elseif ($box1['bp_scopeofWork'] == "Erection") {
            $scopeID = 2;
        } elseif ($box1['bp_scopeofWork'] == "Addition") {
            $scopeID = 3; 
        } elseif ($box1['bp_scopeofWork'] == "Alteration") {
            $scopeID = 4; 
        } elseif ($box1['bp_scopeofWork'] == "Renovation") {
            $scopeID = 5; 
        } elseif ($box1['bp_scopeofWork'] == "Conversion") {
            $scopeID = 6; 
        } elseif ($box1['bp_scopeofWork'] == "Repair") {
            $scopeID = 7; 
        } elseif ($box1['bp_scopeofWork'] == "Moving") {
            $scopeID = 8; 
        } elseif ($box1['bp_scopeofWork'] == "Raising") {
            $scopeID = 9; 
        } elseif ($box1['bp_scopeofWork'] == "Accessory Building/Structure") {
            $scopeID = 10;
        } else {
            $scopeID = 11;   
        };

        if ($box1['bp_characterOccupancy'] == "Group A: Residential, Dwellings") {
            $occupancyID = 1;  
            $occupancyClassified = 'RESIDENTIAL';        
        } elseif ($box1['bp_characterOccupancy'] == "Group B: Residential Hotel, Apartment") {
            $occupancyID = 2;
            $occupancyClassified = 'RESIDENTIAL'; 
        } elseif ($box1['bp_characterOccupancy'] == "Group C: Educational, Recreational") {
            $occupancyID = 3;
            $occupancyClassified = 'EDUCATIONAL';  
        } elseif ($box1['bp_characterOccupancy'] == "Group D: Institutional") {
            $occupancyID = 4;
            $occupancyClassified = 'INSTITUTIONAL'; 
        } elseif ($box1['bp_characterOccupancy'] == "Group E: Business and Mercantile") {
            $occupancyID = 5;
            $occupancyClassified = 'COMMERCIAL';
        } elseif ($box1['bp_characterOccupancy'] == "Group F: Industrial") {
            $occupancyID = 6;
            $occupancyClassified = 'INDUSTRIAL'; 
        } elseif ($box1['bp_characterOccupancy'] == "Group G: Industrial Storage and Hazardous") {
            $occupancyID = 7;
            $occupancyClassified = 'INDUSTRIAL'; 
        } elseif ($box1['bp_characterOccupancy'] == "Group H: Recreational, Assembly Occupant Load Less Than 1000") {
            $occupancyID = 8;
            $occupancyClassified = 'RECREATIONAL';
        } elseif ($box1['bp_characterOccupancy'] == "Group I: Recreational, Assembly Occupant Load 1000 or More") {
            $occupancyID = 9;
            $occupancyClassified = 'RECREATIONAL';
        } elseif ($box1['bp_characterOccupancy'] == "Group J: Agricultural Accessory") {
            $occupancyID = 10;
            $occupancyClassified = 'AGRICULTURAL';
        } else{
            $occupancyID = 11;
            $occupancyClassified = 'OTHERS';
        };                        
        $newBuildingDetails = array(                          
            'application_no' => $buildingID,
            'zoning_no' => 0,
            'subd_no' => 0,
            'date_of_entry' => Date(Now()),
            'area_no_prov_code' => $buildingMain['bp_applicationNo'],
            'area_no_bgry_code' => $buildingMain['bp_areaNo'],          
            'establishment_name' => $appName,
            'scope_of_work' => $scopeID,
            'char_of_occupancy' => $occupancyID,
            'occupancy_classified' => $occupancyClassified,           
            'no_of_storey' => $box1['bp_noOfUnits'],
            'no_of_units' => $box1['bp_noOfUnits'],
            'total_estmtd_cst' => $box1['bp_totalEstCost'],           
            'total_flr_area' => $box1['bp_totalfloorArea'],           
            'proposed_date_of_const' => $box1['bp_dateofConstruction'],
            'expected_date_of_comp' => $box1['bp_dateofCompletion'],            
            'bldg_classification' => $occupancyClassified,
          );                   
        DB::table($this->lgu_db.'.eceo_application_details')->insert($newBuildingDetails);
        $buildingAppInfo = array(        
            'application_no' => $buildingID,
            'owner_id' => $appID, 
            'owner_prefix' => $box1['bp_applicantPFix'],  
            'owner_firstname' => $box1['bp_applicantFName'], 
            'owner_middlename' => $box1['bp_applicantMName'], 
            'owner_lastname' => $box1['bp_applicantLName'], 
            'owner_suffix' => $box1['bp_applicantSFix'], 
            'owner_tin' => $box1['bp_applicantTIN'], 
            'owner_type' => $appType, 
            'owner_ppid' => $box1['bp_applicantID'],
            'owner_bid' => $box1['bp_ownedConstruction'],
            'owner_businessname' => $box1['bp_ownedConstruction'],
            'owner_businesstype' => $box1['bp_formOwnership'], 
            'brgy_id' => $box1['bp_addBarangayID'], 
            'addr_no' => $box1['bp_addNo'], 
            'addr_st' => $box1['bp_addStreet'], 
            'addr_brgy' => $box1['bp_addBarangay'], 
            'addr_citymun' => $box1['bp_addCityMun'], 
            'addr_zipcode' => $box1['bp_zipCode'], 
            'tel_no' => $box1['bp_telNo'], 
            'occupancy' => $occupancyClassified,
          );                  
        DB::table($this->lgu_db.'.eceo_application_info')->insert($buildingAppInfo);
        $buildingScopeInfo = array(        
            'application_no' => $buildingID,
            'scope_id' => $scopeID, 
            'scope_name' => $box1['bp_scopeofWork'],  
            'scope_remarks' => $box1['bp_scopeofRemarks'],           
          );                  
        DB::table($this->lgu_db.'.eceo_bldg_scope')->insert($buildingScopeInfo);
        $buildingNatureInfo = array(        
            'application_no' => $buildingID,
            'nature_id' => $occupancyID, 
            'nature_name' => $box1['bp_characterOccupancy'],  
            'nature_remarks' => $occupancyClassified,          
          );        
        DB::table($this->lgu_db.'.eceo_bldg_nature')->insert($buildingNatureInfo);
        if ($box2['bp_architectId'] > 0) {
            $engineerID = $box2['bp_architectId'];           
            } else {
            $engineerID = 0;
            };  
        $buildingEngineer = array(       
            'application_no' => $buildingID,
            'person_id' => $engineerID,
            'title_no' => 3,
            'person_name' => $box2['bp_architectName'],
            'st_no' => $box2['bp_architectAddress'],
            'sig_date' => $box2['bp_architectDatesigned'],
            'sig_prc_no' => $box2['bp_architectPRCNo'],
            'sig_prc_date' => $box2['bp_architectValidityDate'],
            'sig_ptr_no' => $box2['bp_architectPTRNo'],
            'sig_ptr_date' => $box2['bp_architectDateissued'],
            'sig_ptr_place' => $box2['bp_architectIssuedat'],
            'sig_tin_no' => $box2['bp_architectTIN'],
            'sig_ctc_no' => $box5['bp_signArchCTCNo'],
            'sig_ctc_date' => $box5['bp_signArchDate'],
            'sig_ctc_place' => $box5['bp_signArchPlaceIssued'],
            'validity' => '',
            'sig_type' => 'Civil Engr',
          );                  
        DB::table($this->lgu_db.'.eceo_signatory_persons')->insert($buildingEngineer);
        if ($box3['bp_applicantconfirmId'] > 0) {
            $applicantID = $box3['bp_applicantconfirmId'];           
            } else {
            $applicantID = 0;
            };  
        $buildingApplicant = array(        
            'application_no' => $buildingID,
            'person_id' => $applicantID,
            'title_no' => 1,
            'person_name' => $box3['bp_applicantconfirmName'],
            'st_no' => $box3['bp_applicantconfirmAddress'],
            'sig_date' => $box3['bp_applicantdateApplied'],
            'sig_prc_no' => '',
            'sig_prc_date' => $box3['bp_applicantdateApplied'],
            'sig_ptr_no' => '',
            'sig_ptr_date' => $box3['bp_applicantdateApplied'],
            'sig_ptr_place' => '',
            'sig_tin_no' => $box1['bp_applicantTIN'],
            'sig_ctc_no' => $box3['bp_applicantconfirmCTCNo'],
            'sig_ctc_date' => $box3['bp_applicantdateIssued'],
            'sig_ctc_place' => $box3['bp_applicantconfirmplaceIssued'],
            'validity' => '',
            'sig_type' => 'Applicant',
          );                        
        DB::table($this->lgu_db.'.eceo_signatory_persons')->insert($buildingApplicant);
        if ($box4['bp_lotownerID'] > 0) {
            $lotOwnerID = $box4['bp_lotownerID'];           
            } else {
            $lotOwnerID = 0;
            };  
        $buildingLotOwner = array(        
            'application_no' => $buildingID,
            'person_id' => $lotOwnerID,
            'title_no' => 2,
            'person_name' => $box4['bp_lotownerName'],
            'st_no' => $box4['bp_lotownerAddress'],
            'sig_date' => $box4['bp_lotownerDate'],
            'sig_prc_no' => '',
            'sig_prc_date' => $box4['bp_lotownerDate'],
            'sig_ptr_no' => '',
            'sig_ptr_date' => $box4['bp_lotownerDate'],
            'sig_ptr_place' => '',
            'sig_tin_no' => '',
            'sig_ctc_no' => $box4['bp_lotownerCTCNo'],
            'sig_ctc_date' => $box4['bp_lotownerDateissued'],
            'sig_ctc_place' => $box4['bp_lotownerPlaceissued'],
            'validity' => '',
            'sig_type' => 'Lot Owner',
          );                         
        DB::table($this->lgu_db.'.eceo_signatory_persons')->insert($buildingLotOwner);
        $buildingApplication = array(     
            'application_no' => $buildingID,
            'notary_date' => $box5['bp_signOnDate'],
            'notary_id' => 0,
            'notary_name' => $box5['bp_signNotaryPublic'],
            'notary_year' => $box5['bp_signYear'],
            'doc_no' => $box5['bp_signDocNo'],
            'page_no' => $box5['bp_signPageNo'],
            'book_no' => $box5['bp_signBookNo'],
            'series_no' => $box5['bp_signSeriesOf'],
            'notary_citymun' => $box5['bp_signCityMun'],
            'notary_location' => $box5['bp_signCityMun'],
          );                                
        DB::table($this->lgu_db.'.eceo_application_bldg')->insert($buildingApplication);                  
        
          $Wood = "NO";
          $Concrete = "NO";
          $Steel = "NO";
          $Mixed = "NO";
        foreach ($materialsDetails as $row) {     
                  if ($row == "Wood") {
                    $Wood = 'YES';           
                  } elseif ($row == "Concrete") {
                    $Concrete = 'YES';
        ...
        $buildingCost = array(     
            ...
            'mat_mixed' => $Mixed,
            'mat_others' => $Others,
            'mat_specify' => $box1['bp_othersSpecify'],
            'cost1' => $box1['bp_buildingFee'],
            'cost2' => $box1['bp_electricalFee'],
            'cost3' => $box1['bp_mechanicalFee'],
            'cost4' => $box1['bp_plumbingFee'],
            'cost5' => $box1['bp_sanitaryFee'],
            'cost6' => $box1['bp_othersFee'],
            'cost_total' => $box1['bp_totalEstCost'],
          );                          
        DB::table($this->lgu_db.'.eceo_application_cost')->insert($buildingCost);
        if ($box7['bp_issuanceNo'] == "") {
            $Status = 'PENDING';           
        } else {
            $Status = 'ISSUED'; 
        };   
        $buildingPermit = array(
            'app_no' => $buildingID,
            'bp_status' => $Status,
            'bp_id' => $box7['bp_issuanceNo'],
            'date_issued' => $box7['bp_issuanceDate'],
            'bp_type' => 'A',
            'or_no' => $box7['bp_issuanceOfficialNo'],
            'or_date' => $box7['bp_issuanceDatepaid'],
            'issued_to' => $box7['bp_issuanceOwnername'],
            'bldg_name' => $box7['bp_issuanceProjecttype'],
            'issued_by' => $box7['bp_issuancedeptname'],
            'issued_id' => $box7['bp_issuancedeptnameID'],
            'project_id' => $buildingMain['bp_projectID'],
            'project_name' => $buildingMain['bp_projectName'],
          );                      
        DB::table($this->lgu_db.'.eceo_building_permit')->insert($buildingPermit);
        // foreach ($box6 as $row) {       
        //     $array=array(              
        //         'payer_type' => $appType,
        //         'payer_id' => $appID,
        ...
        //         'bill_description'=> $row['IncomeAcount'],
        //         'net_amount' => $row['Amount Due'],
        //         'bill_amount' => $row['Amount Due'],
        //         'bill_month' => Date(Now()),
        //         'bill_number' =>$buildingMain['bp_applicationNo'],
        //         'transaction_type' => 'Building Permit',
        //         'ref_id' => $buildingID,
        //         'bill_id' => $buildingID,
        //         'include_from' => 'Others',
        //         'assested_by' => $row['Assessed By'],               
        //         'waivestatus' => '',
        //         'nso' => $row['NSO'],                                        
        //    );                       
    ...
    public function update($idx,$buildingMain,$box1,$materialsDetails,$box2, $box3, $box4, $box5, $box6, $box7)
    { 
        if ($box1['bp_ownedConstructionID'] > 0) {
            $appType = 'BUSINESS';
            $appID = $box1['bp_ownedConstructionID'];
        ...
        DB::table($this->lgu_db.'.eceo_application')->where('application_no',$idx)->update($newBuilding);
        if ($box1['bp_scopeofWork'] == "New Construction") {
            $scopeID = 1;          
        } elseif ($box1['bp_scopeofWork'] == "Erection") {
            $scopeID = 2;
        ...
        DB::table($this->lgu_db.'.eceo_application_details')->where('application_no',$idx)->update($newBuildingDetails);
        $buildingAppInfo = array(        
            'application_no' => $idx,
            'owner_id' => $appID, 
            'owner_prefix' => $box1['bp_applicantPFix'],  
        ...
        DB::table($this->lgu_db.'.eceo_application_info')->where('application_no',$idx)->update($buildingAppInfo);
        $buildingScopeInfo = array(        
            'application_no' => $idx,
            'scope_id' => $scopeID, 
            'scope_name' => $box1['bp_scopeofWork'],  
        ...
        DB::table($this->lgu_db.'.eceo_bldg_scope')->where('application_no',$idx)->update($buildingScopeInfo);
        $buildingNatureInfo = array(        
            'application_no' => $idx,
            'nature_id' => $occupancyID, 
            'nature_name' => $box1['bp_characterOccupancy'],  
        ...
        DB::table($this->lgu_db.'.eceo_bldg_nature')->where('application_no',$idx)->update($buildingNatureInfo);
        if ($box2['bp_architectId'] > 0) {
            $engineerID = $box2['bp_architectId'];           
            } else {
            $engineerID = 0;
        ...
        DB::table($this->lgu_db.'.eceo_signatory_persons')->where('application_no',$idx)->where('sig_type','Civil Engr')->update($buildingEngineer);
        if ($box3['bp_applicantconfirmId'] > 0) {
            $applicantID = $box3['bp_applicantconfirmId'];           
            } else {
            $applicantID = 0;
        ...
        DB::table($this->lgu_db.'.eceo_signatory_persons')->where('application_no',$idx)->where('sig_type','Applicant')->update($buildingApplicant);
        if ($box4['bp_lotownerID'] > 0) {
            $lotOwnerID = $box4['bp_lotownerID'];           
            } else {
            $lotOwnerID = 0;
        ...
        DB::table($this->lgu_db.'.eceo_signatory_persons')->where('application_no',$idx)->where('sig_type','Lot Owner')->update($buildingLotOwner);
        $buildingApplication = array(     
            'application_no' => $idx,
            'notary_date' => $box5['bp_signOnDate'],
            'notary_id' => 0,
        ...
        DB::table($this->lgu_db.'.eceo_application_bldg')->where('application_no',$idx)->update($buildingApplication);
          $Wood = "NO";
          $Concrete = "NO";
          $Steel = "NO";
          $Mixed = "NO";
        ...
        DB::table($this->lgu_db.'.eceo_application_cost')->where('application_no',$idx)->update($buildingCost);
        if ($box7['bp_issuanceNo'] == "") {
            $Status = 'PENDING';           
        } else {
            $Status = 'ISSUED'; 
        ...
        DB::table($this->lgu_db.'.eceo_building_permit')->where('app_no',$idx)->update($buildingPermit);
        // foreach ($box6 as $row) {       
        //     $array=array(              
        //         'payer_type' => $appType,
        //         'payer_id' => $appID,
    ...
    {  
        ...
        $reason['Form_name'] ='Building Permit';       
        $reason['Trans_ID'] =$id;       
    ...
    public function printBuildingPermitList(Request $request)
    {
        $data = $request->buildingList;        
        $logo = config('variable.logo');        
        try {
            $html_content ='<body>
            '.$logo.'            
            <h2 align="center">Building Permit Master List</2>
            <br></br>
            <br></br>
            <table border="1" cellpadding="2">
                ...
                foreach($data as $row){                             
                    ...
                    <tr>
                        ...
                        <td align="center" style="width:6%">'.$row['BP No'].'</td>
