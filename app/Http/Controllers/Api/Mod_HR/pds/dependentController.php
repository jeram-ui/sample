...
class dependentController extends Controller
{
  ...
  public function __construct(GlobalController $global)
    ...
    public function showDependent(Request $request)
    {
      $list = DB::table($this->hr_db . '.employees_dependent')
   // ->join($this->hr_db .'.employee_information','employee_information.PPID','employees.SysPK_Empl')
    ...
    public function dependentStore(Request $request)
    {
      $form = $request->form;
      $id = $form['id'];
      $form['SysPK_Empl']=Auth::user()->Employee_id;
      if ( $id >0 ) {
        DB::table($this->hr_db . '.employees_dependent')
        ->where("id",$id)
        ->update($form);
      }else{
        $form['SysPK_Empl']=Auth::user()->Employee_id;
    ...
    public function dependCancel($id)
    {
        db::table($this->hr_db . '.employees_dependent')
            ->where('id', $id)
            ->update(['status' => 1]);
