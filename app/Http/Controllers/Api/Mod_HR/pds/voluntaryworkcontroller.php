...
class voluntaryworkcontroller extends Controller
{
  ...
  public function __construct(GlobalController $global)
    ...
    public function voluntarywork(Request $request)
    {
      $list = DB::table($this->hr_db . '.employees_voluntarilywork')
   // ->join($this->hr_db .'.employee_information','employee_information.PPID','employees.SysPK_Empl')
    ...
    public function storeVol(Request $request)
    {
      ...
      if ( $id >0 ) {
        DB::table($this->hr_db . '.employees_voluntarilywork')
        ->where("id",$id)
        ->update($form);
      }else{
        $form['emp_number']=Auth::user()->Employee_id;
    ...
    public function voluntaryworkCancel($id)
    {
        db::table($this->hr_db . '.employees_voluntarilywork')
            ->where('id', $id)
            ->update(['status' => 1]);
