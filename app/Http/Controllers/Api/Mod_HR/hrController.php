...
class hrcontroller extends Controller
{
  ...
  public function printingDtr($data, $date)
  {
    ...
    //   $equivalentUndertime = $value->equivalentUndertime;
    //   $equivalentLate = $value->equivalentlate;
    // }
    ...
    $overtimeDB= db::select("select ".$this->hr_db ."._get_equivalent_day_hrs(?) as overtime",[$totalOvertime * 60]);
    foreach ($overtimeDB as $key => $valueo) {
      log::Debug($valueo->overtime);
      $totalOvertimeEqui = $valueo->overtime;
    }
    $html = '
