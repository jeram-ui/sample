...
class BoneChanberController extends Controller
{
    private $lgu_db;
    private $hr_db;
    private $trk_db;
    ...
    public function displayCalendar(Request $request)
    {
        ...
        $empid = Auth::user()->id;
        $mapping = DB::table($lgu_db . '.sched_group_mapping')->where('sched_name', 'Bone Chamber')->first();
        $data_calendar = DB::table($lgu_db . '.appointments')
            ->where('Status', 0)
            ->where('appointments.no_years', '5 years')
        ...
        $data_calendar = DB::table($lgu_db . '.appointments')
            ->where('Status', 0)
            ->where('appointments.no_years', '10 years')
            ->where('sched_group', $mapping->group_id)
            ->whereBetween('appointments.StartDate', [$request->from, $request->to])
            ->orderBy("trans_date", "asc")->get();
          ...
          ' . $logo . '
            ...
            <tr >
                <th style="width:100%" align="Center">
                    <h3>INTERMENT FORM-NAALAD CEMETERY</h3>
                </th>
            </tr>
            <br>
            ...
            <tr style="height:25px">
               <th style="width:5%"></th>
               <th style="width:30%"><b>Nova D. Matin-ao</b></th>
               <th style="width:60%" align = "right" >( Look For Raul N. Cañete ) </th>
               <th style="width:5%"></th>
            </tr>
            <tr style="height:25px">
                <th style="width:5%"></th>
                <th style="width:30%"> <b>BPLO-Staff</b></th>
